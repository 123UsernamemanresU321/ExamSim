...
Because real live-site credentials and designated test accounts were not provided, this pass did not perform active internet testing, account creation, brute force simulation, storage probing, SMS/email sends, or live data access. The assessment below is a static/local code and configuration review of the Exam Vault repository, with concrete low-risk reproduction plans for the actual website using synthetic accounts.

## Test Account Matrix

These accounts were requested by the checklist but were not created because the live asset details were placeholders.

| Account | Status | Notes |
| --- | --- | --- |
| Unauthenticated visitor | Not created | Covered by route/static inspection only. |
| Normal user A | Pending live test setup | Should be a student profile assigned to a test assessment. |
| Normal user B | Pending live test setup | Needed for IDOR/BOLA validation against user A. |
| Suspended/banned user | Not found in model | No clear suspended/banned account state was found in the local scan. |
| Unverified user | Pending live test setup | Relevant to activation/login flows. |
| Verified user | Pending live test setup | Student and owner variants needed. |
| Moderator | Not applicable in current model | Marker assignment exists, but no separate moderator auth role was identified. |
| Admin/owner | Pending live test setup | Owner with AAL2 required for sensitive owner functions. |
...
| EV-SEC-008 | Info | Dynamic launch testing gap | This report could not validate actual website behavior, account separation, browser headers on deployed domains, storage bucket runtime settings, or RLS using real policies because URLs/accounts were placeholders. | Provide the actual website URL and synthetic accounts, then run the dynamic plan below. | Scope placeholders in the request. | Launch sign-off should include live low-risk validation with synthetic data. | Run the live dynamic test plan and attach evidence before production launch. | CI/checklist item requiring completed dynamic security QA artifact before launch. |
...
- Dynamic testing is pending because actual website test accounts were not supplied; this is acceptable only if completed before launch.
...
## Recommended Dynamic Live Test Plan

Run these only against the actual website using synthetic test accounts:
...
Current status: not launch-ready for production until EV-SEC-001 is fixed and EV-SEC-002 is either fixed or explicitly accepted only for low-stakes pilots. After those fixes, run the dynamic live plan with synthetic accounts and attach evidence before production launch.
