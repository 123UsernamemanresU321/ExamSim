...
serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "delete") {
      ...
      const { error } = await admin.from("comment_bank_items").delete().eq("id", body.id).eq("owner_profile_id", ownerProfile.id);
      if (error) throw error;
      await auditOwnerAction(ownerProfile.id, user.id, "comment_bank.deleted", "comment_bank_items", body.id);
      return json({ ok: true });
    }
    ...
    if (body.action === "use") {
      ...
      const { data, error } = await admin
        ...
        .single();
      if (error) throw error;
      await auditOwnerAction(ownerProfile.id, user.id, "comment_bank.used", "comment_bank_items", body.id);
    ...
    const query = body.id
      ? admin.from("comment_bank_items").update(payload).eq("id", body.id).eq("owner_profile_id", ownerProfile.id)
      : admin.from("comment_bank_items").insert(payload);
    const { data, error } = await query.select("*").single();
    if (error) throw error;
    await auditOwnerAction(ownerProfile.id, user.id, body.id ? "comment_bank.updated" : "comment_bank.created", "comment_bank_items", data.id);
    return json({ ok: true, item: data });
  } catch (error) {
