...
import { StudentResultsWorkspace } from "./student-results-workspace";
import { createSupabaseBrowserClient } from "@/lib/supabase/browser";
import { getStudentAttemptResultsWorkspaceClient } from "@/lib/live-data-client";
import type { AttemptReviewWorkspace } from "@/lib/live-data";

export function StudentResultsClient({ id }: { id: string }) {
  const [workspace, setWorkspace] = useState<AttemptReviewWorkspace | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const data = await getStudentAttemptResultsWorkspaceClient(id, supabase);
        if (!data) {
    ...
    return (
      <div className="flex min-h-screen flex-col bg-white">
        ...
        <div className="flex flex-1 items-center justify-center">
          <p className="text-[var(--muted)] animate-pulse">Loading assessment results...</p>
        </div>
      </div>
    );
  }

  if (error || !workspace?.attempt) {
    return (
      <div className="flex min-h-screen flex-col bg-white">
        <AppHeader />
        <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
          <h2 className="text-xl font-bold text-red-600">Error</h2>
          <p className="mt-2 text-[var(--muted)]">{error || "Attempt not found."}</p>
        </div>
      </div>
    );
  }
