  ...
  return (
    <div className="rounded-md border border-[var(--border)] bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.14em] text-[var(--subtle)]">Released score filter</p>
      ...
      {active === "overall" ? (
        <div className="mt-4 flex items-center justify-between rounded-md bg-[var(--surface-muted)] p-4">
          ...
          <span className="text-2xl font-semibold text-[var(--ink)]">{overallScore === null ? "No data" : `${overallScore}%`}</span>
        </div>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          {visibleGroups.map((group) => (
            <div key={`${group.kind}-${group.key}`} className="rounded-md border border-[var(--border)] p-3">
              ...
              <p className="mt-3 text-2xl font-semibold text-[var(--ink)]">{group.average_released_score}%</p>
            </div>
          ))}
        </div>
      ) : (
