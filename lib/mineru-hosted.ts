...
export type MineruBatchRequestInput = {
  dataId: string;
  signedUrl?: string;
  fileName?: string;
  modelVersion?: string;
...
export type MineruResultState = "pending" | "running" | "done" | "failed" | "unknown";
...
export function buildMineruBatchRequest(input: MineruBatchRequestInput): MineruBatchRequest {
  const uploadMode = input.uploadMode ?? "signed_url";
  if (uploadMode === "signed_url" && !input.signedUrl) throw new Error("signedUrl is required for signed_url MinerU uploads");
  if (uploadMode === "file_upload" && !input.fileName) throw new Error("fileName is required for file_upload MinerU uploads");
...
export function pickMineruExtractResult(raw: unknown, dataId: string): MineruExtractResult {
  ...
  const selected =
    ...
    candidates[0] ??
    {};
  const state = normalizeState(stringValue(selected.state ?? selected.status ?? data.state ?? data.status));
  return {
    state,
...
function normalizeState(value: string | null): MineruResultState {
