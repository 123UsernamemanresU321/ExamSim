...
export function getExamsimProductionReadiness(env: ExamsimReadinessEnv = process.env) {
  ...
  const items: ExamsimProductionReadinessItem[] = [
    ...
    },
    {
      ...
      status: "manual_fallback",
      ownerMessage: "Allowed materials and basic response controls exist; advanced graphing/geometry/CAS tools must stay labelled unavailable until integrated.",
      productionPath: "Teacher enables calculator/formula booklet/drawing/table/sketch tools per session policy.",
      fallback: "Use allowed-materials drawer, PDF uploads, typed answers, and manual working uploads.",
      requiredEnvVars: [],
      qaChecklist: ["Verify allowed materials policy", "Hide unavailable tools", "Check student exam UI does not overclaim"],
    },
    {
    ...
    },
    {
      ...
      status: "manual_fallback",
      ownerMessage: "QTI import/export exists conservatively; Moodle XML needs validation and unsupported-feature warnings before production use.",
      productionPath: "Export/import without losing marks, response types, topics, or source metadata silently.",
      fallback: "Use normalized JSON import/export and QTI where supported; warn on unsupported Moodle/XML features.",
      requiredEnvVars: [],
      qaChecklist: ["Export QTI", "Import QTI", "Check unsupported-feature warnings", "Validate normalized JSON"],
    },
    {
    ...
    },
    {
      ...
      status: "staging_required",
      ownerMessage: "Cohort/group dashboards have foundations; school-level reporting must be validated for cross-workspace permissions and export correctness.",
      productionPath: "Show group comparisons, completion rates, topic weaknesses, support flags, progress, and exports.",
      fallback: "Use cohort filters, marking queue, and CSV exports until school reporting is validated at scale.",
      requiredEnvVars: [],
