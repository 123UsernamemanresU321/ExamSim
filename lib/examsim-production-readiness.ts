...
export function getExamsimProductionReadiness(env: ExamsimReadinessEnv = process.env) {
  ...
  const items: Array<Omit<ExamsimProductionReadinessItem, "evidence">> = [
    ...
    },
    {
      ...
      status: "live_validation_required",
      ownerMessage: "Per-student and session policies cover server-enforced extra time, upload extensions, access windows, rest breaks, display preferences, calculator/formula policies, and allowed materials. TTS stays visibly unavailable without a real provider.",
