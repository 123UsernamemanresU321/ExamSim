  ...
  if (isDemoModeEnabled() && assessmentId === sampleAssessment.id) {
    return {
      ...
      versions: [
        {
          ...
          markscheme_source_kind: null,
          markscheme_source_object_path: null,
          published_at: null,
          created_at: sampleAssessment.created_at,
        },
      ],
      ...
      questionNodes: samplePackage.questions.map((node) => ({
        ...
        markscheme_html: node.markscheme_html ?? null,
        markscheme_pdf_path: node.markscheme_pdf_path ?? null,
        assets: node.assets ?? [],
...
export async function getStudentAttemptResultsWorkspace(attemptId: string): Promise<AttemptReviewWorkspace> {
  if (isDemoModeEnabled() && attemptId.startsWith("att_")) return getOwnerAttemptReviewWorkspace(attemptId);
  return await invokeEdgeFunctionServer<AttemptReviewWorkspace>("get-student-results", { attempt_id: attemptId });
}
